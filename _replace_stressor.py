...
OLD = """def _apply_image_stressor(img, stressor_key):
...
NEW = """def _apply_image_stressor(img, stressor_key, image_domain="general"):
    \"\"\"Apply domain-appropriate stressor to the image.\"\"\"
    arr = np.array(img, dtype=np.float32)
    w, h = img.size

    # ── Medical stressors ────────────────────────────────────────────────────
    if stressor_key == "low_contrast":
        # Reduce contrast — simulate underexposure
        arr = arr * random.uniform(0.3, 0.55) + random.uniform(5, 20)
        img = Image.fromarray(np.clip(arr, 0, 255).astype(np.uint8))

    elif stressor_key == "image_noise":
        # Gaussian sensor noise — simulate MRI/CT noise
        noise_std = random.uniform(20, 50)
        noise = np.random.normal(0, noise_std, arr.shape)
        arr = arr + noise
        img = Image.fromarray(np.clip(arr, 0, 255).astype(np.uint8))

    elif stressor_key == "compression_artifact":
        # JPEG compression artifacts — simulate lossy transmission
        import io
        ...
        # Simulate different scanner intensity distribution
        ...
        # Directional motion blur — simulate patient movement
        radius = random.randint(6, 18)
        img = img.filter(ImageFilter.GaussianBlur(radius=radius))

    elif stressor_key == "staining_variation":
        # Hue/saturation shift — simulate staining inconsistency
        from PIL import ImageEnhance
...
if OLD in content:
    ...
    with open(path, "w", encoding="utf-8") as f:
        f.write(content)
    print("_apply_image_stressor replaced. Size:", os.path.getsize(path))
else:
    print("ERROR: old string not found")
    idx = content.find("def _apply_image_stressor")
    print("Function at char:", idx)
